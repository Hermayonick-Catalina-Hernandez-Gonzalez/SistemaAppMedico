<nav x-data="{ open: false }" class="custom-navbar">
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if (Auth::user()->role === 'Secretario')
                        ...
                        <x-nav-link href="{{ route('secretario.registro-productos') }}" :active="request()->routeIs('secretario.registro-productos')"
                            class="text-black">
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden ">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if (Auth::user()->role === 'Administrador')
                <x-responsive-nav-link href="{{ route('admin.registro-pacientes') }}" :active="request()->routeIs('admin.registro-pacientes')"
                    class="text-black">
                    {{ __('Pacientes') }}
                </x-responsive-nav-link>

                <x-responsive-nav-link href="{{ route('admin.registro-medicos') }}" :active="request()->routeIs('admin.registro-medicos')"
                    class="text-black">
                    {{ __('Médicos') }}
                </x-responsive-nav-link>

                <x-responsive-nav-link href="{{ route('admin.registro-secretarios') }}" :active="request()->routeIs('admin.registro-secretarios')"
                    class="text-black">
                    {{ __('Secretarios') }}
                </x-responsive-nav-link>

                <x-responsive-nav-link href="{{ route('admin.registro-servicios') }}" :active="request()->routeIs('admin.registro-servicios')"
                    class="text-black">
                    {{ __('Servicios') }}
                </x-responsive-nav-link>

                <x-responsive-nav-link href="{{ route('admin.registro-productos') }}" :active="request()->routeIs('admin.registro-productos')"
                    class="text-black">
                    {{ __('Productos') }}
                </x-responsive-nav-link>

                <x-responsive-nav-link href="{{ route('admin.registro-enfermeros') }}" :active="request()->routeIs('admin.registro-enfermeros')"
                    class="text-black">
                    {{ __('Enfermeros') }}
                </x-responsive-nav-link>
            @endif
            ...
            @if (Auth::user()->role === 'Secretario')
                <x-responsive-nav-link href="{{ route('secretario.registro-pacientes') }}" :active="request()->routeIs('secretario.registro-pacientes')"
                    class="text-black">
                    {{ __('Registro Pacientes') }}
                </x-responsive-nav-link>

                <x-responsive-nav-link href="{{ route('secretario.consultas') }}" :active="request()->routeIs('secretario.consultas')"
                    class="text-black">
                    {{ __('Citas Agendadas') }}
                </x-responsive-nav-link>

                <x-responsive-nav-link href="{{ route('secretario.registro-productos') }}" :active="request()->routeIs('secretario.pagos')"
                    class="text-black">
                    {{ __('Registrar Productos') }}
                </x-responsive-nav-link>
            @endif
        ...
        <div class="pt-4 pb-1 border-t border-gray-200 background-color: #3FBBB4">
            <div class="px-4">
                <div class="font-medium text-base text-black">{{ Auth::user()->nombre }} {{ Auth::user()->apellido }}
                </div>
